...
def spotify_callback(request, format=None):
    ...
    print('session in spotify_callback ', request.session, request.session.session_key, session_id)
    ...
    print('before session_id ', session_id)
    # if not request.session.exists(request.session.session_key):
    #     request.session.create()
...
class IsAuthenticated(APIView):
    def get(self, request, format=None):
        ...
        api_views.getSessionId = self.request.session
        print('IsAuthenticated  in spotify views.py ', request.session, self.request.session, type(self.request.session), self.request.session.session_key, type(self.request.session.session_key))
        print(session_id)
